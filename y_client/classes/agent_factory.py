...
def create_agent_data(
    ...
    **kwargs
    ...
    base_config = {
        ...
        'attention_window': int(config['agents'].get('attention_window', 5)),
        'probability_of_daily_follow': float(config['agents'].get('probability_of_daily_follow', 0.0)),
        'probability_of_secondary_follow': float(config['agents'].get('probability_of_secondary_follow', 0.0)),
        'emotions': config['posts'].get('emotions', []),
        'actions_likelihood': config['simulation'].get('actions_likelihood', {}),
        'opinion_dynamics': config['simulation'].get('opinion_dynamics'),
        'llm_v_config': {
            'url': config['servers'].get('llm_v', ''),
